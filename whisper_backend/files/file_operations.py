...
def sync_database_with_directory(
    ...
    :param root_path: Абсолютный путь к корневой директории файловой системе.
    ...
    # Для файлов используем `parent_directory` для фильтрации по директории,
    ...
    try:
        ...
        # Если директория не существует, выводим сообщение
    ...
    for entry in file_system_entries:
        ...
        if os.path.isfile(full_path):  # Если элемент — это файл.
            if entry not in existing_files:
                ...
                # Удаляем его из словаря `existing_files`,
                #   так как он больше не требует обработки.
                existing_files.pop(entry)

    # === Удаление отсутствующих директорий ===
    for directory_name, directory_obj in existing_directories.items():
        # Все оставшиеся директории в `existing_directories`
        #   отсутствуют в файловой системе.
        # Удаляем их из базы данных, используя метод `delete_directory`
        #   из модели Directory.
    ...
    for file_name, file_obj in existing_files.items():
        # Все оставшиеся файлы в `existing_files` отсутствуют
        #   в файловой системе.
        # Удаляем их из базы данных, используя метод `delete_file`
        #   из модели File.
